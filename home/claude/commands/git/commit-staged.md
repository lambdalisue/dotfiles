...
1. **Analyze** - Use the Task tool (`subagent_type: "git-commit-staged"`) to analyze staged changes and draft a commit message. If the agent reports nothing is staged, inform the user and **STOP**.
...
3. **Execute** - If approved, use the Task tool (`subagent_type: "git-commit-staged"`) to execute the commit with the approved (or edited) message. Present the result to the user.
