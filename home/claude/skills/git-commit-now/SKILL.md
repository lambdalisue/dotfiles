---
name: git-commit-now
description: Analyze working tree changes and commit them WITHOUT asking for approval — fixup into existing commits where appropriate, otherwise new atomic commits
...
This is the non-interactive variant of `/git-commit`. The user invoking
...
2. **Execute** - Execute the plan **directly via the Bash tool** (do NOT delegate to the agent for execution). Do NOT ask for approval — the `/git-commit-now` invocation is the explicit permission.
