---
name: git-commit-new-now
description: Analyze working tree changes, plan logically minimal NEW atomic commits per hunk (no fixup), and execute them WITHOUT asking for approval
...
This is the non-interactive variant of `/git-commit-new`. The user invoking
...
2. **Execute** - Execute the plan **directly via the Bash tool** (do NOT delegate to the agent for execution). Do NOT ask for approval — the `/git-commit-new-now` invocation is the explicit permission.
