...
When creating a GitHub Issue with `gh as -q gh issue create`, always set the issue type afterward using the REST API:

```bash
gh as -q gh api -X PATCH repos/{owner}/{repo}/issues/{number} --field type={type}
```
